...
# داخل قسم elif search_type == "الصيانة الوقائية":
if not df_filtered.empty:
    ...
    if 'التاريخ_التالي' in df_filtered.columns:
        ...
        df_filtered = df_filtered.sort_values(by='التاريخ_التالي', ascending=True)  # تصاعدي = من الأقرب للأبعد
    
    st.success(f"تم العثور على {len(df_filtered)} مهمة صيانة")
    st.dataframe(df_filtered, use_container_width=True)
    ... # باقي الكود (تحميل Excel)
...
# ------------------------------- دوال خدمة الماكينات (جديدة) -------------------------------
def normalize_name(s: str) -> str:
    """تطبيع النصوص للمقارنة (إزالة المسافات الزائدة، تحويل إلى صغير، إزالة رموز خاصة)"""
    if s is None:
        return ""
    s = str(s).replace("\n", "+")
...
def split_needed_services(needed_service_str):
    """تقسيم قائمة الخدمات المطلوبة (مفصولة بـ + أو , أو سطر جديد)"""
...
def highlight_cell(val, col_name):
    """تنسيق ألوان الخلايا حسب اسم العمود"""
    color_map = {
...
def get_servised_by_value(row):
    """استخراج اسم فني الخدمة من أعمدة متعددة محتملة"""
...
def show_service_statistics(needed, done, remaining):
    """عرض إحصائيات الخدمة: عدد الخدمات الكلي، المنجز، المتبقي"""
    total = len(needed)
    completed = len(done)
    pending = len(remaining)
    col1, col2, col3 = st.columns(3)
    col1.metric("📋 إجمالي الخدمات المطلوبة", total)
    col2.metric("✅ الخدمات المنفذة", completed, delta=f"{completed/total*100:.0f}%" if total>0 else "0%")
    col3.metric("⏳ الخدمات المتبقية", pending, delta=f"-{pending}" if pending>0 else "")

def check_service_status(card_num, current_tons, all_sheets):
    """
    الفحص الأساسي لحالة الخدمة لآلة معينة.
    
    Parameters:
    - card_num: رقم الماكينة (رقم أو نص)
    - current_tons: عدد الأطنان الحالية (رقم)
    - all_sheets: قاموس يحوي جميع أوراق Excel (sheet_name -> DataFrame)
    """
    # البحث عن ورقة تحتوي على بيانات الخدمة (يفترض اسم sheet "Service" أو "Data")
    target_sheet = None
    for sheet_name, df in all_sheets.items():
        if "service" in sheet_name.lower() or "data" in sheet_name.lower():
            target_sheet = df
            break
    if target_sheet is None:
        # إذا لم نجد، نستخدم أول ورقة
        target_sheet = list(all_sheets.values())[0]
    
    # توحيد أسماء الأعمدة (باستخدام normalize_name)
    ...
    card_col = None
    tones_col = None
    service_needed_col = None
    service_done_col = None
    last_service_date_col = None
    
    for col in df.columns:
        if "card" in col or "machine" in col or "رقم" in col or "ماكينة" in col:
            card_col = col
        if "ton" in col or "طن" in col or "tones" in col:
            tones_col = col
        if "service needed" in col or "خدمة مطلوبة" in col:
            service_needed_col = col
        if "service done" in col or "خدمة تمت" in col:
            service_done_col = col
        if "date" in col and "service" in col or "تاريخ" in col:
            last_service_date_col = col
    
    # التحقق من وجود العمود الأساسي لرقم الماكينة
    if card_col is None:
        st.error("لم يتم العثور على عمود يحدد رقم الماكينة في البيانات.")
        return
    
    # البحث عن صف الماكينة
    try:
        card_num_str = str(card_num)
        row = df[df[card_col].astype(str).str.strip() == card_num_str]
        if row.empty:
            st.warning(f"⚠️ لم يتم العثور على الماكينة رقم {card_num}")
            return
        row = row.iloc[0]
    except Exception as e:
        st.error(f"خطأ في البحث عن الماكينة: {e}")
        return
    
    # استخراج قيم الخدمة
    needed_services_str = row.get(service_needed_col, "") if service_needed_col else ""
    done_services_str = row.get(service_done_col, "") if service_done_col else ""
    last_date = row.get(last_service_date_col, "") if last_service_date_col else ""
    target_tons = row.get(tones_col, 0) if tones_col else 0
    
    # تحويل الأطنان إلى رقم
    try:
        target_tons = float(target_tons) if target_tons else 0
        current_tons = float(current_tons)
    except:
        target_tons = 0
    
    needed_list = split_needed_services(needed_services_str)
    done_list = split_needed_services(done_services_str)
    
    # تحديد الخدمات المتبقية (لم تتم بعد)
    remaining_services = [s for s in needed_list if s not in done_list]
    
    # تحديد الحالة
    if current_tons >= target_tons and target_tons > 0:
        if remaining_services:
            status = "Service Needed"
            message = f"⚠️ الخدمة مطلوبة: {', '.join(remaining_services)}"
        else:
            status = "Service Done"
            message = "✅ جميع الخدمات المطلوبة تمت."
    else:
        if remaining_services:
            status = "Service Didn't Done"
            message = f"❌ لم تتم الخدمات التالية: {', '.join(remaining_services)}"
        else:
            status = "Service Done (No pending)"
            message = "لا توجد خدمات معلقة."
    
    # بناء جدول النتائج
    result_data = {
        "رقم الماكينة": card_num,
        "الأطنان الحالية": current_tons,
        "الأطنان المستهدفة للخدمة": target_tons,
        "الخدمات المطلوبة": ", ".join(needed_list) if needed_list else "-",
        "الخدمات المنفذة": ", ".join(done_list) if done_list else "-",
        "الخدمات المتبقية": ", ".join(remaining_services) if remaining_services else "-",
        "آخر تاريخ خدمة": last_date if last_date else "-",
        "حالة الخدمة": status,
        "فني الخدمة": get_servised_by_value(row)
    }
    ...
    styled_df = result_df.style.applymap(
        lambda x: highlight_cell(x, "Service Needed") if x == "Service Needed" else
                  (highlight_cell(x, "Service Done") if x == "Service Done" else
                   (highlight_cell(x, "Service Didn't Done") if x == "Service Didn't Done" else "")),
        subset=["حالة الخدمة"]
    )
    ...
    # إحصائيات إضافية
    show_service_statistics(needed_list, done_list, remaining_services)
...
# ------------------------------- دوال إدارة البيانات المحسنة (جديدة) -------------------------------
def edit_sheet_with_save_button(sheets_edit):
    """عرض وتحرير أي شيت مع حفظ التغييرات وزر حفظ مخصص"""
    st.subheader("✏️ تحرير الورقة")
    sheet_name = st.selectbox("اختر الورقة:", list(sheets_edit.keys()), key="edit_sheet_name")
    
    df = sheets_edit[sheet_name].astype(str).reset_index(drop=True)
    edited_df = st.data_editor(df, num_rows="dynamic", use_container_width=True, key=f"editor_{sheet_name}")
    
    if st.button(f"💾 حفظ التغييرات في {sheet_name}", type="primary"):
        sheets_edit[sheet_name] = edited_df.astype(object)
        success = save_and_push_to_github(sheets_edit, f"تعديل يدوي في شيت {sheet_name}")
        if success:
            st.success("✅ تم الحفظ والرفع إلى GitHub")
...
def add_new_row(sheets_edit):
    """إضافة صف جديد إلى شيت محدد"""
    st.subheader("➕ إضافة صف جديد")
    sheet_name = st.selectbox("اختر الورقة:", list(sheets_edit.keys()), key="add_row_sheet")
    df = sheets_edit[sheet_name].astype(str).reset_index(drop=True)
    ...
    for i, col in enumerate(df.columns):
        with cols[i % 4]:
            new_row[col] = st.text_input(f"{col}", key=f"new_row_{sheet_name}_{col}")
    
    if st.button("💾 إضافة الصف", type="primary"):
        new_df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        sheets_edit[sheet_name] = new_df.astype(object)
        if save_and_push_to_github(sheets_edit, f"إضافة صف إلى {sheet_name}"):
            st.success("✅ تم إضافة الصف وحفظه")
            st.rerun()
        else:
            st.error("❌ فشل الحفظ")
    return sheets_edit

def add_new_column(sheets_edit):
    """إضافة عمود جديد إلى شيت محدد"""
    st.subheader("➕ إضافة عمود جديد")
    sheet_name = st.selectbox("اختر الورقة:", list(sheets_edit.keys()), key="add_col_sheet")
    ...
    if st.button("💾 إضافة العمود", type="primary") and new_col_name:
        df[new_col_name] = default_value
        sheets_edit[sheet_name] = df.astype(object)
        if save_and_push_to_github(sheets_edit, f"إضافة عمود '{new_col_name}' إلى {sheet_name}"):
            st.success("✅ تم إضافة العمود وحفظه")
            st.rerun()
        else:
            st.error("❌ فشل الحفظ")
    return sheets_edit

def add_new_event_enhanced(sheets_edit):
    """إضافة حدث جديد (مثل عطل أو صيانة) مع تفاصيله"""
    st.subheader("📝 إضافة حدث جديد")
    sheet_name = st.selectbox("اختر الورقة:", list(sheets_edit.keys()), key="add_event_sheet_enh")
    df = sheets_edit[sheet_name].astype(str)
    
    col1, col2 = st.columns(2)
    with col1:
        card_num = st.text_input("رقم الماكينة:")
        event_text = st.text_area("الحدث (العطل/الصيانة):")
    with col2:
        correction_text = st.text_area("التصحيح / الإجراء المتخذ:")
        serviced_by = st.text_input("فني الخدمة:")
    event_date = st.text_input("التاريخ (مثال: 2025-05-20):")
    
    if st.button("💾 إضافة الحدث", type="primary") and card_num:
        new_row = {"card": card_num.strip()}
        if event_date:
            new_row["Date"] = event_date.strip()
        event_col = next((c for c in df.columns if normalize_name(c) in ["event", "الحدث"]), "Event")
        correction_col = next((c for c in df.columns if normalize_name(c) in ["correction", "تصحيح"]), "Correction")
        servised_col = next((c for c in df.columns if "servis" in normalize_name(c) or "فني" in c), "Servised by")
        
        new_row[event_col] = event_text.strip()
        new_row[correction_col] = correction_text.strip()
        new_row[servised_col] = serviced_by.strip()
        
        new_df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        sheets_edit[sheet_name] = new_df.astype(object)
        if save_and_push_to_github(sheets_edit, f"إضافة حدث إلى {sheet_name}"):
            st.success("✅ تم إضافة الحدث وحفظه")
            st.rerun()
        else:
            st.error("❌ فشل الحفظ")
    return sheets_edit
...
def edit_events_and_corrections(sheets_edit):
    """تعديل الأحداث والتصحيحات لصف محدد"""
    st.subheader("✏️ تعديل الأحداث والتصحيحات")
    sheet_name = st.selectbox("اختر الورقة:", list(sheets_edit.keys()), key="edit_events_sheet")
    df = sheets_edit[sheet_name].astype(str).reset_index(drop=True)
    
    event_col = next((c for c in df.columns if normalize_name(c) in ["event", "الحدث"]), None)
    correction_col = next((c for c in df.columns if normalize_name(c) in ["correction", "تصحيح"]), None)
    display_cols = ["card", "Date"]
    if event_col:
        display_cols.append(event_col)
    if correction_col:
        display_cols.append(correction_col)
    st.dataframe(df[display_cols], use_container_width=True)
    
    row_idx = st.number_input("رقم الصف المراد تعديله (0-based):", min_value=0, max_value=len(df)-1, step=1)
    if st.button("تحميل البيانات"):
        st.session_state["edit_row_data"] = df.iloc[row_idx].to_dict()
        st.session_state["edit_row_idx"] = row_idx
    
    if "edit_row_data" in st.session_state:
        data = st.session_state["edit_row_data"]
        idx = st.session_state["edit_row_idx"]
        new_card = st.text_input("رقم الماكينة:", value=data.get("card", ""))
        new_date = st.text_input("التاريخ:", value=data.get("Date", ""))
        new_event = st.text_area("الحدث:", value=data.get(event_col, "") if event_col else "")
        new_correction = st.text_area("التصحيح:", value=data.get(correction_col, "") if correction_col else "")
        
        if st.button("💾 حفظ التعديلات", type="primary"):
            df.at[idx, "card"] = new_card
            df.at[idx, "Date"] = new_date
            if event_col:
                df.at[idx, event_col] = new_event
            if correction_col:
                df.at[idx, correction_col] = new_correction
            sheets_edit[sheet_name] = df.astype(object)
            if save_and_push_to_github(sheets_edit, f"تعديل حدث في {sheet_name} الصف {idx}"):
...
def manage_data_editor(sheets_edit):
    """
    الوظيفة الرئيسية لإدارة وتعديل البيانات.
    تعرض واجهة كاملة للمستخدم تحتوي على عدة تبويبات.
    """
    if sheets_edit is None:
        st.error("لا توجد بيانات متاحة للتعديل. يرجى تحميل الملف أولاً.")
        return sheets_edit
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        ...
        "📝 إضافة حدث جديد", "✏️ تعديل الأحداث والتصحيحات"
    ])
    
    with tab1:
        sheets_edit = edit_sheet_with_save_button(sheets_edit)
    with tab2:
        sheets_edit = add_new_row(sheets_edit)
    with tab3:
        sheets_edit = add_new_column(sheets_edit)
    with tab4:
        sheets_edit = add_new_event_enhanced(sheets_edit)
    with tab5:
        sheets_edit = edit_events_and_corrections(sheets_edit)
    
    return sheets_edit

# ------------------------------- دالة إدارة البيانات الأساسية (معدلة لاستخدام المحسن) -------------------------------
def add_new_department(sheets_edit):
    if st.session_state.get("username") == "admin":
        st.subheader("➕ إضافة قسم جديد")
        st.info("سيتم إنشاء قسم جديد (شيت جديد) في ملف Excel لإدارة ماكينات هذا القسم")
...
tabs_list = ["🔍 بحث متقدم", "📊 تحليل الأعطال", "🔔 الإشعارات", "🔧 حالة الخدمة"]
if can_edit:
    tabs_list.append("🛠 تعديل وإدارة البيانات")
...
# تبويب حالة الخدمة (جديد)
with tabs[3]:
    st.header("🔧 حالة خدمة الماكينات")
    st.markdown("""
    **الوصف:** هذا التبويب يسمح لك بالتحقق من الخدمات المطلوبة لكل ماكينة بناءً على الأطنان المنقولة.
    يجب أن يحتوي ملف Excel على ورقة تحتوي على أعمدة مثل: رقم الماكينة، الأطنان المستهدفة، الخدمات المطلوبة، الخدمات المنفذة، تاريخ آخر خدمة.
    """)
    with st.form(key="service_check_form"):
        col1, col2 = st.columns(2)
        with col1:
            machine_number = st.text_input("🔢 رقم الماكينة:", placeholder="مثال: 101 أو M-22")
        with col2:
            current_tons = st.number_input("⚖️ الأطنان الحالية:", min_value=0.0, step=0.1, format="%.1f")
        submitted = st.form_submit_button("🔍 تحقق من حالة الخدمة", type="primary")
    if submitted and machine_number:
        try:
            if machine_number.isdigit():
                machine_number = int(machine_number)
            check_service_status(machine_number, current_tons, all_sheets)
        except Exception as e:
            st.error(f"حدث خطأ: {e}")

# تبويب تعديل وإدارة البيانات
if can_edit and len(tabs) > 4:
    with tabs[4]:
        # يمكن اختيار إما واجهة الإدارة الأصلية أو المحسنة. هنا نستخدم المحسنة.
        sheets_edit = manage_data_editor(sheets_edit)
