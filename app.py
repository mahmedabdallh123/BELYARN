...
APP_CONFIG = {
    ...
    "GENERAL_SECTION": "عام",
    # ========== NEW ==========
    "PRODUCTS_SHEET": "المنتجات",
    "PRODUCTS_COLUMNS": ["اسم المنتج", "المشرف", "الوزن", "نوع المنتج", "تاريخ_الإضافة", "رابط_الصورة"]
}
...
def load_products():
    if not os.path.exists(APP_CONFIG["LOCAL_FILE"]):
        return pd.DataFrame(columns=APP_CONFIG["PRODUCTS_COLUMNS"])
    try:
        df = pd.read_excel(APP_CONFIG["LOCAL_FILE"], sheet_name=APP_CONFIG["PRODUCTS_SHEET"])
        df.columns = df.columns.astype(str).str.strip()
        for col in APP_CONFIG["PRODUCTS_COLUMNS"]:
            if col not in df.columns:
                df[col] = ""
        return df.fillna("")
    except Exception:
...
def get_all_sections_from_excel():
    ...
    if not sheets:
        return []
    sections = [name for name in sheets.keys() if name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]]
    return sections
...
def get_allowed_sections(all_sheets, username, required_permission="view"):
    ...
    for sheet_name in all_sheets.keys():
        if sheet_name in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]:
            continue
...
def save_excel_locally(sheets_dict):
    try:
        ...
        if APP_CONFIG["PRODUCTS_SHEET"] not in sheets_dict:
            sheets_dict[APP_CONFIG["PRODUCTS_SHEET"]] = load_products()
...
def search_across_sheets(all_sheets):
    ...
    search_type = st.selectbox("نوع البيانات المراد البحث فيها:", ["الأقسام (الأعطال)", "قطع الغيار", "الصيانة الوقائية", "المنتجات"], key="search_type")
    ...
    # ===================== NEW: بحث في المنتجات =====================
    elif search_type == "المنتجات":
        products_df = load_products()
        if products_df.empty:
            st.warning("لا توجد منتجات مسجلة")
            return
        df_filtered = products_df.copy()
        search_term = st.text_input("🔍 كلمة البحث (الاسم، النوع، المشرف...):", key="search_term_products")
        if search_term:
            mask = pd.Series([False] * len(df_filtered))
            search_columns = ["اسم المنتج", "نوع المنتج", "المشرف"]
            for col in search_columns:
                if col in df_filtered.columns:
                    mask = mask | df_filtered[col].astype(str).str.contains(search_term, case=False, na=False)
            df_filtered = df_filtered[mask]
        if not df_filtered.empty:
            st.success(f"تم العثور على {len(df_filtered)} منتج")
            st.dataframe(df_filtered, use_container_width=True)
            excel_file = export_filtered_results_to_excel(df_filtered, "المنتجات")
            st.download_button("📥 تحميل النتائج", excel_file, f"products_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        else:
            st.warning("لا توجد نتائج")
...
def add_new_department(sheets_edit):
    if st.session_state.get("username") == "admin":
        ...
        deletable_sections = [name for name in sheets_edit.keys() if name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]]
    ...
    if sheets_edit:
        for dept_name in sheets_edit.keys():
            if dept_name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]:
...
def add_maintenance_as_event(sheets_edit, equipment_name, task_name, execution_date, performed_by, used_spare_part="", used_quantity=1, image_url=None):
    ...
    target_df = None
    for sheet_name, df in sheets_edit.items():
        if sheet_name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]:
...
# ===================== NEW: تبويب المنتجات =====================
def products_management_tab(sheets_edit):
    st.header("📦 إدارة المنتجات")
    
    if APP_CONFIG["PRODUCTS_SHEET"] not in sheets_edit:
        sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]] = pd.DataFrame(columns=APP_CONFIG["PRODUCTS_COLUMNS"])
    
    tab_add, tab_view = st.tabs(["➕ إضافة منتج", "📋 عرض المنتجات"])
    
    with tab_add:
        with st.form("add_product_form"):
            col1, col2 = st.columns(2)
            with col1:
                product_name = st.text_input("🏷️ اسم المنتج *")
                supervisor = st.text_input("👤 المشرف")
                weight = st.text_input("⚖️ الوزن (كجم / وحدة)")
            with col2:
                product_type = st.text_input("📂 نوع المنتج")
                product_image = st.file_uploader("🖼️ صورة المنتج (اختياري)", type=APP_CONFIG["ALLOWED_IMAGE_TYPES"])
            ...
            if submitted:
                if not product_name:
                    ...
                    if product_image is not None:
                        ...
                        image_url = upload_image_to_github(product_image, "product", prod_id)
                        if image_url:
                            st.success("✅ تم رفع الصورة")
                        else:
                            st.warning("⚠️ فشل رفع الصورة، سيتم الحفظ بدونها")
                    ...
                    new_row = pd.DataFrame([{
                        ...
                        "تاريخ_الإضافة": datetime.now().strftime("%Y-%m-%d %H:%M"),
                        "رابط_الصورة": image_url or ""
                    }])
                    
                    df_products = sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]]
                    sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]] = pd.concat([df_products, new_row], ignore_index=True)
                    
                    if save_and_push_to_github(sheets_edit, f"إضافة منتج: {product_name}"):
                        log_activity("add_product", f"تم إضافة منتج '{product_name}'")
    ...
    with tab_view:
        df = sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]].copy()
        
        if df.empty:
            st.info("ℹ️ لا توجد منتجات مسجلة حتى الآن.")
            return sheets_edit
        
        col_f1, col_f2 = st.columns(2)
        with col_f1:
            search_term = st.text_input("🔍 بحث في الاسم أو النوع:", key="product_search")
        with col_f2:
            type_filter = st.selectbox("🏷️ فلتر بنوع المنتج:", ["الكل"] + sorted(df["نوع المنتج"].dropna().unique()), key="product_type_filter")
        
        if search_term:
            mask = df["اسم المنتج"].astype(str).str.contains(search_term, case=False) | df["نوع المنتج"].astype(str).str.contains(search_term, case=False)
            df = df[mask]
        if type_filter != "الكل":
            df = df[df["نوع المنتج"] == type_filter]
        
        st.markdown(f"**عدد المنتجات: {len(df)}**")
        
        view_mode = st.radio("طريقة العرض:", ["جدول", "بطاقات مع الصور"], horizontal=True, key="product_view_mode")
        
        if view_mode == "جدول":
            display_cols = [c for c in df.columns if c != "رابط_الصورة"]
                        ...
                        with col:
                            with st.container(border=True):
                                ...
                                if img_url and str(img_url).strip():
                                    ...
                                    st.write("📦 لا توجد صورة")
                                st.markdown(f"**🏷️ {row['اسم المنتج']}**")
                                st.markdown(f"👤 **المشرف:** {row['المشرف']}")
                                st.markdown(f"⚖️ **الوزن:** {row['الوزن']}")
                                st.markdown(f"📂 **النوع:** {row['نوع المنتج']}")
                                st.caption(f"📅 {row['تاريخ_الإضافة']}")
        
        if st.session_state.get("username") == "admin":
            st.markdown("---")
            st.subheader("🛠️ تعديل أو حذف منتج")
            product_list = df["اسم المنتج"].tolist()
            if product_list:
                selected_prod = st.selectbox("اختر المنتج:", product_list, key="edit_product_select")
                if selected_prod:
                    row_data = df[df["اسم المنتج"] == selected_prod].iloc[0]
                    original_idx = row_data.name
                    
                    with st.expander(f"✏️ تعديل {selected_prod}"):
                        new_name = st.text_input("اسم المنتج", value=row_data["اسم المنتج"], key="edit_p_name")
                        new_sup = st.text_input("المشرف", value=row_data["المشرف"], key="edit_p_sup")
                        new_weight = st.text_input("الوزن", value=row_data["الوزن"], key="edit_p_weight")
                        new_type = st.text_input("نوع المنتج", value=row_data["نوع المنتج"], key="edit_p_type")
                        
                        if st.button("💾 حفظ التعديلات", key="save_product_edit"):
                            df_products = sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]]
                            df_products.loc[original_idx, "اسم المنتج"] = new_name
                            df_products.loc[original_idx, "المشرف"] = new_sup
                            df_products.loc[original_idx, "الوزن"] = new_weight
                            df_products.loc[original_idx, "نوع المنتج"] = new_type
                            sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]] = df_products
                            if save_and_push_to_github(sheets_edit, f"تعديل منتج: {selected_prod}"):
                                st.success("✅ تم التعديل")
                                st.rerun()
                    
                    if st.button(f"🗑️ حذف {selected_prod} نهائياً", key="delete_product_btn"):
                        df_products = sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]]
                        df_products = df_products.drop(index=original_idx)
                        sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]] = df_products
                        if save_and_push_to_github(sheets_edit, f"حذف منتج: {selected_prod}"):
                            st.success("✅ تم الحذف")
                            st.rerun()
        else:
            st.info("🔒 التعديل والحذف متاحان فقط للمسؤول (admin).")
...
def manage_data_edit(sheets_edit):
    ...
    if APP_CONFIG["PRODUCTS_SHEET"] not in sheets_edit:
        sheets_edit[APP_CONFIG["PRODUCTS_SHEET"]] = load_products()
    
    tab_names = ["📋 عرض الأقسام", "🔧 إدارة الماكينات", "➕ إضافة قسم جديد", "📦 قطع الغيار", "🛠 الصيانة الوقائية", "📦 المنتجات"]
    tabs_edit = st.tabs(tab_names)
    with tabs_edit[0]:
        st.subheader("جميع الأقسام")
        if sheets_edit:
            dept_names = [name for name in sheets_edit.keys() if name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]]
            if dept_names:
    ...
    with tabs_edit[1]:
        if sheets_edit:
            sheet_name = st.selectbox("اختر القسم:", [name for name in sheets_edit.keys() if name not in [APP_CONFIG["SPARE_PARTS_SHEET"], APP_CONFIG["MAINTENANCE_SHEET"], APP_CONFIG["PRODUCTS_SHEET"]]], key="manage_machines_sheet_edit")
    ...
    with tabs_edit[5]:
        sheets_edit = products_management_tab(sheets_edit)
    return sheets_edit
